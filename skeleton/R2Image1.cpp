...
float* weights = (float*)malloc(kernelSize * sizeof(float));

double denominator = (1 / sqrt(2 * PI * sigma * sigma));
double numerator;
double totalWeight = 0.0;

for (int i = -kernelSize / 2; i < kernelSize / 2; i++) {
numerator = exp(-1 * ((i*i) / (2 * sigma * sigma)));
weights[i + kernelSize / 2] = numerator * denominator;
totalWeight += weights[i + kernelSize / 2];
}

//for (int m = -kernelSize / 2; m < kernelSize / 2; m++) {
//	weights[m + kernelSize / 2] /= totalWeight;
//}

R2Pixel newPix, nullPixel;

for (int y = kernelSize / 2; y < height - kernelSize / 2; y++) {
for (int x = 0; x < width; x++) {
newPix = nullPixel;
for (int i = -kernelSize / 2; i <= kernelSize / 2; i++) {
newPix += weights[i + kernelSize / 2] * Pixel(x, y + i);
}
temp.Pixel(x, y) = newPix;
}
}
...
newPix += weights[j + kernelSize / 2] * temp.Pixel(x + j, y);
}
Pixel(x, y) = newPix;
}
}
...
void FeatureDetectionOld(R2Image &img, priority_queue<pixCompare, vector<pixCompare>, Compare> pq) {
	...
	int selectedSoFar = 0;
	int pixelsApart = 10;
	pixCompare selectedFeatures[140];
	int index = 0;
	...
	while (selectedSoFar < COUNT) {
		...
		if (!blank.Pixel(x_coord, y_coord).IsBlack()) {
			selectedFeatures[index] = currPix;
			index++;
			selectedSoFar++;
...
Harris(double sigma)
{
	...
	for (int x = 0; x < width; x++) {
		for (int y = 0; y < height; y++) {
			...
			SetPixel(x, y, tempPix);
		}
	}

	priority_queue<pixCompare, vector<pixCompare>, Compare> pq;
	pq = MakePriorityQueue(*this);
...
pixCompare computeSSD(pixCompare feature, R2Image &img, R2Image &other) {
	int soFarSSD = INFINITY;
	int tempSSD = 0;
	int diff = 0;
	int x_coord = feature.x;
	int y_coord = feature.y;

	int searchWidthSpan = img.Width;
	int searchHeightSpan = img.Height;
	...
	// loop through the search window of 20% image size
	for (int dx = max(0, x_coord - searchWidthSpan); dx < min(img.Width, x_coord + searchWidthSpan); dx++) {
		for (int dy = max(0, y_coord - searchHeightSpan); dy < min(img.Height, y_coord + searchHeightSpan); dy++) {

			// inside that window, go through all possible 21x21 'feature windows' and compute their SSD
			for (i = dx; i < min(searchWidthSpan, dx + featureSpan); i++) {
				for (j = dy; j < min(searchHeightSpan, dy + featureSpan); j++) {
					diff = (img.Pixel(i, j).Blue + img.Pixel(i, j).Red + img.Pixel(i, j).Green) -
						(other.Pixel(i, j).Blue + other.Pixel(i, j).Red + other.Pixel(i, j).Green);
					tempSSD += diff * diff;
				}
			}
			// save the coordinates of the central pixel with lowest feature window SSD
			// thus, the best candidate for the feature matching
			if (tempSSD < soFarSSD) {
				match_x = i / 2;
				match_y = j / 2;
			}
			soFarSSD = min(soFarSSD, tempSSD);
		}
	}
