...
public:
	...
	void HighPass(double sigma);

	void MarkFeatures(pixCompare * features);
	pixCompare* FeatureDetectionNew(R2Image &img, priority_queue<pixCompare, vector<pixCompare>, Compare> pq);
	void Harris(double sigma);

	void Sharpen(void);
	void line(int x0, int x1, int y0, int y1, float r, float g, float b);
	void testHomography();
	void computeBestHomography(R2Image * otherImage, double * bestMatrix);
	boolean * dltRansac(R2Image * otherImage);
	void ApplyMatrix(double** h);
	static double RankHomography(pair<int, int> *feature, pair<int, int> *matched, int ignore, int size);
	void blendWithOther(R2Image *otherImage);
	void BlendWithMask(R2Image* mask, R2Image* white);

	// further operations
	pair <int, int> finalTrack(R2Image * otherImage, pair <int, int>, int featureWidth, int featureHeight);
	void MarkFeatures(pair<int, int> * features, int pixelsApart, int size);
	pixCompare* FeatureDetectionFinal(R2Image &img, priority_queue<pixCompare, vector<pixCompare>, Compare> pq);
	pixCompare* CollectFeatures();
	static double** computeHomographyFinal(pair<int, int> *from, pair<int, int> *to);

	static double** computeHomographyAgain(pair<int, int> *from, pair<int, int> *to);

	void blendOtherImageTranslated(R2Image * otherImage);
	void blendOtherImageHomography(R2Image * otherImage);
	void initialBlendOtherImageHomography(R2Image * otherImage);
